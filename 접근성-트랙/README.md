# 접근성 트랙 — 장애가 있으셔도 코딩하기

> 시각·청각·운동·인지 접근성을 다룹니다. 자경단은 이 골목을 칸막이로 두지 않습니다.

안녕하세요. "정상" 코딩 환경이라는 건 없다고 봅니다. 본인 방식이 맞는 환경입니다.

## 학습 순서

| 챕터 | 제목 | 상태 |
|------|------|------|
| 0 | [준비물 — 본인의 보조기기 점검](./00-준비물.md) | ✅ |
| 1 | [macOS·Windows 화면읽기 (VoiceOver / 내레이터) 켜기](./01-화면읽기.md) | ✅ |
| 2 | [한 손 키보드 / 음성 코딩 (Talon, Cursorless) 입문](./02-음성-코딩.md) | ✅ |
| 3 | [큰 글씨 + 고대비 화면 셋업](./03-큰글씨-고대비.md) | ✅ |
| 4 | [자막 자동 생성 도구로 강의 영상 보기](./04-자막.md) | ✅ |
| 5 | [내가 만든 웹사이트 접근성 점수 매기기](./05-접근성-점수.md) | ✅ |
| 6 | [접근성 자체를 첫 쥐돌이로 만들기 (이슈 발견 → PR)](./06-접근성-쥐돌이.md) | ✅ |

## 학습 원칙

- **본인 방식이 우선입니다.** "정상" 코딩 환경이라는 건 없다고 봅니다.
- **속도를 강요하지 않습니다.** 챕터 하나가 한 달이어도 괜찮습니다.
- **본인 도구는 본인이 가장 잘 아십니다.** 멘토는 학습자에게 배우는 자세로 임합니다.

## 멘토 자경단원분들께

> [!TIP]
> GitHub은 스크린리더(VoiceOver/NVDA/내레이터)를 비교적 잘 지원합니다. 학습자가 처음이시면 [`GITHUB-GUIDE.md`](../GITHUB-GUIDE.md) §1~5(읽기·issue까지)부터 안내해 주세요. 키보드만으로 거의 모든 작업이 가능합니다.

- 본인이 잘 모르는 보조기기 사용자에게 "이렇게 해보세요"라고 권하시는 건 위험할 수 있습니다. 학습자의 워크플로우를 먼저 들어주세요.
- 접근성은 **새끼고양이의 권리**라고 봅니다. 시혜가 아닙니다 ([`MANIFESTO`](https://github.com/catguard-team/manifesto/blob/main/MANIFESTO.md)).
- 외부 자료는 한국어를 우선하지만, 영어가 더 풍부한 영역입니다. 양쪽 다 안내해 주세요.

## 참고 자료 (자경단 검증 안 됨, 출처 표시용)

- [한국웹접근성평가센터](https://www.wa.or.kr) — 정부 운영
- [ACCESSIBILITY 한국 커뮤니티](https://accessibility.kr) (있으시면 채워주세요)
- [Talon Voice](https://talonvoice.com) — 음성 코딩
- [W3C WAI](https://www.w3.org/WAI/)

---

**고맙습니다. 냐-옹.**
